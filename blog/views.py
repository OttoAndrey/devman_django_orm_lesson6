...
def index(request):
    ...
    fresh_posts = Post.objects.annotate(num_com=Count('comments')).order_by('-published_at')
    most_fresh_posts = fresh_posts[:5].prefetch_related('author').fetch_with_tags()
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all()[:20].annotate(num_com=Count('comments')).fetch_with_tags()
